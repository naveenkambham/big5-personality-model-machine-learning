...
Hyper parameters are tuned and set.
"""
...
#this method does the binning based on value ranges. Since the trait value ranges are 0,1 dividing in to three bins
...
def SVCModel(trait):
    ...
    #converting the continuous values in to bins and getting the input and output features
    dataframe[trait] = dataframe[trait].apply(ClassLabel)
    X = dataframe.loc[:,'mediaUsage':'Scheduling_OfficeTools_Weather'].values
    Y = dataframe.loc[:,trait].values

    # applying scaling and model in pipeline
    seed=7
